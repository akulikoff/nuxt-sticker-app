...
export function useCatImages(): CatImagesComposable {
   ...
   */
  const fetchSingleCatImage = async (
    retries = 3,
    ...
    try {
      ...
      try {
        ...
        if (validImages.length === 0) {
          const errorMsg =
            errors.length > 0
              ? `Failed to fetch any images. Errors: ${errors.join(', ')}`
              : 'Failed to fetch any cat images'
        ...
        const errorMessage =
          fallbackErr instanceof Error ? fallbackErr.message : 'Failed to fetch cat images'
